...
class Demande {
    ...
    /** Toutes les demandes ouvertes ou en discussion (pour les prestataires, sans filtre compétences). */
    public function allOuvertes(): array {
        return $this->db->query("
            SELECT d.*, u.nom AS client_nom, u.prenom AS client_prenom,
                   sc.libelle AS category_libelle,
                   (SELECT COUNT(*) FROM proposition p WHERE p.id_demande = d.id_demande AND p.statut = 'envoyee') AS nb_propositions
            FROM demande d
            JOIN \"user\" u ON u.id_user = d.id_user
            JOIN service_category sc ON sc.id_category = d.id_category
            WHERE d.statut IN ('ouverte', 'en_discussion')
            ORDER BY d.urgence DESC, d.published_at DESC
        ")->fetchAll();
    }
