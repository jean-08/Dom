...
class AvisController {
    ...
    public function create(): void {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Retrouver la demande liée pour la redirection
            $id_demande_redirect = 0;
            if ($id_intervention) {
                $intervObj = $this->interv->find($id_intervention);
                $id_demande_redirect = $intervObj ? (int)$intervObj['id_demande'] : 0;
            }
            $redirect = $id_demande_redirect
                ? 'index.php?action=demande_show&id=' . $id_demande_redirect
                : 'index.php?action=demandes';
            ...
            $_SESSION['success'] = $resultat === 'ok' ? 'Avis publié. Merci pour votre retour !' : null;
