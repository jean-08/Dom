...
$active = 'dashboard';
$nbDemandesAvecPropositions = count(array_filter($mesDemandes, fn($d) => (int) ($d['nb_propositions_actives'] ?? 0) > 0));
$nbPropositionsActives = array_sum(array_map(fn($d) => (int) ($d['nb_propositions_actives'] ?? 0), $mesDemandes));
$nbPropositionsTotal = array_sum(array_map(fn($d) => (int) ($d['nb_propositions_total'] ?? 0), $mesDemandes));
...
<?php if ($nbDemandesAvecPropositions > 0): ?>
  <div class="da-summary-card p-3 p-md-4 mb-3">
    <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3">
      <div class="summary-title">
        <span class="summary-icon"><i class="bi bi-envelope-paper-heart"></i></span>
        <div>
          <h5 class="mb-1">Vous avez des propositions reçues</h5>
          <p class="mb-0 text-muted"><?= $nbDemandesAvecPropositions ?> demande<?= $nbDemandesAvecPropositions > 1 ? 's' : '' ?> ont reçu des propositions.</p>
        </div>
      </div>
      <div class="d-flex gap-3 flex-wrap">
        <div class="summary-item">
          <div class="value mb-1"><?= $nbPropositionsActives ?></div>
          <div class="label">Propositions actives</div>
        </div>
        <div class="summary-item">
          <div class="value mb-1"><?= $nbPropositionsTotal ?></div>
          <div class="label">Propositions totales</div>
        </div>
      </div>
      <a href="index.php?action=demandes" class="btn btn-sm btn-brand">Voir mes demandes</a>
    </div>
  </div>
<?php endif; ?>
...
<div class="row g-3">
  ...
  <div class="col-lg-6">
    ...
    <div class="da-card p-3 p-md-4 mb-3">
      <div class="d-flex align-items-center justify-content-between mb-3">
    ...
    <div class="da-card p-3 p-md-4">
      <div class="d-flex align-items-center justify-content-between mb-3">
        <h5 class="mb-0">Mes propositions en cours</h5>
        <a href="index.php?action=demandes" class="small text-decoration-none">Voir tout <i class="bi bi-arrow-right"></i></a>
      </div>
      ...
      <?php else: ?>
        <div class="table-responsive">
          <table class="table table-da align-middle mb-0">
            <thead><tr><th>Titre / Client</th><th>Ma Proposition</th><th></th></tr></thead>
            <tbody>
            <?php foreach (array_slice($mesPropositions, 0, 5) as $dp): ?>
              <?php
              ...
              <tr>
                <td>
                  <strong><?= htmlspecialchars($dp['titre'] ?? mb_strimwidth($dp['description'], 0, 30, '…')) ?></strong>
                  <div class="small text-muted"><?= htmlspecialchars(($dp['client_prenom'] ?? '') . ' ' . ($dp['client_nom'] ?? '')) ?></div>
                </td>
                <td>
                ...
                <td class="text-end">
                  <a href="index.php?action=demande_show&id=<?= (int)$dp['id_demande'] ?>" class="btn btn-sm btn-outline-secondary">
                    <i class="bi bi-eye"></i>
                  </a>
                </td>
              </tr>
