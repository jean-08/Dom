      ...
      <form method="POST" action="index.php?action=prestataire_candidater" enctype="multipart/form-data">
        ...
        <div class="mb-4">
          ...
          <div class="row g-2">
            <?php foreach (($categories ?? []) as $cat): ?>
              <?php
                $catId = (int)($cat['id_category'] ?? $cat['id_service'] ?? 0);
                $catLabel = $cat['libelle'] ?? $cat['nom'] ?? 'Service';
              ?>
              <div class="col-md-6">
                <div class="form-check p-2 border rounded bg-light">
                  <input class="form-check-input ms-1 me-2" type="checkbox" name="categories[]" value="<?= $catId ?>" id="cat_<?= $catId ?>">
                  <label class="form-check-label fw-medium" for="cat_<?= $catId ?>">
        ...
        <div class="row mb-3">
          ...
          <div class="col-md-6">
            ...
            <select name="moyen_deplacement" class="form-select">
              <option value="Vehicule personnel" <?= ($_POST['moyen_deplacement'] ?? $profilExistant['moyen_deplacement'] ?? '') === 'Vehicule personnel' ? 'selected' : '' ?>>Véhicule personnel</option>
        ...
        <div class="mb-3">
          ...
          <input type="text" name="zone_intervention" class="form-control"
                 placeholder="Ex : Tana, Ankatso, rayon 30km autour de l'Université d'Antananarivo"
