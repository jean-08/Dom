...
class DemandeController {
    ...
    public function show(): void {
        ...
        if ($role === 'admin') {
            ...
            // Prestataire retenu OU prestataire ayant une proposition active
            $profil = $this->prest->findByUser((int)($user['id_user'] ?? 0));
            if ($profil) {
                $id_profile = (int)$profil['id_prestataire'];
                // Accès si prestataire retenu
                if (!empty($demande['id_profile_retenu'])
                    && (int)$demande['id_profile_retenu'] === $id_profile
                ) {
                    $ok = true;
                }
                // Accès si proposition envoyée sur cette demande
        ...
        $propositions         = $this->proposition->byDemande($id);
    ...
    public function disponibles(): void {
        ...
        // eligibles() filtre par compétences, zone, et statut de la demande
        $demandes = $this->demande->eligibles((int)$prest['id_prestataire']);
    ...
    public function proposer(): void {
        ...
        if ($resultat === 'ok') {
            ...
            if ($demandeInfo && !empty($demandeInfo['id_user'])) {
                ...
                (new Notification())->nouvelleProposition((int)$demandeInfo['id_user'], $id_demande, $demandeInfo['titre'] ?? 'demande', $nomPrest);
            }
        } else {
            match ($resultat) {
        ...
        header('Location: index.php?action=demandes_disponibles'); exit;
    }
    ...
    public function selectionner(): void {
        ...
        if ($resultat === 'ok') {
            ...
            if ($propInfo && $demandeInfo && !empty($propInfo['id_user'])) {
                (new Notification())->propositionRetenue((int)$propInfo['id_user'], $id_demande, $demandeInfo['titre'] ?? '');
            }
        } else {
            match ($resultat) {
    ...
    public function confirmerEngagement(): void {
        ...
        if ($resultat === 'ok') {
            ...
            if ($demandeInfo && !empty($demandeInfo['id_user'])) {
                $nomPrest = trim(($prest['prenom'] ?? '') . ' ' . ($prest['nom'] ?? ''));
                (new Notification())->engagementConfirme((int)$demandeInfo['id_user'], $id_demande, $demandeInfo['titre'] ?? '', $nomPrest);
            }
        } else {
            match ($resultat) {
