...
BEGIN
    ...
    WHERE d.statut IN ('ouverte', 'en_discussion')
      -- Correspondance de compétence
      AND d.id_category IN (
          SELECT c.id_category FROM competence c
          WHERE c.id_profile = p_id_profile
      )
...
BEGIN
    ...
    -- Remettre la demande en discussion (ou ouverte si plus de propositions)
    UPDATE proposition SET statut = 'non_retenue', decided_at = now()
    WHERE id_demande = p_id_demande AND id_profile = p_id_profile AND statut = 'retenue';

    UPDATE demande
    SET statut = 'annulee_par_prestataire',
