...
class Solution {
    ...
    public function valider(int $id_solution, int $id_user): string {
        ...
        try {
            ...
            if ($row['validee_par_client'] || $row['statut'] === 'solution_proposee' || $row['statut'] === 'intervention_planifiee' || $row['statut'] === 'intervention_en_cours' || $row['statut'] === 'terminee' || $row['statut'] === 'cloturee') {
                $this->db->rollBack();
                return 'deja_validee';
            }
            if ($row['statut'] !== 'diagnostic_propose') {
                $this->db->rollBack();
                return 'refuse';
            }
    ...
    public function refuser(int $id_solution, int $id_user): string {
        ...
        try {
            ...
            if ($row['validee_par_client'] || $row['refusee_par_client'] || $row['statut'] !== 'diagnostic_propose') {
                $this->db->rollBack();
                return 'deja_traitee';
            }
            ...
            $updDem = $this->db->prepare("UPDATE demande SET statut = 'engagee' WHERE id_demande = ?");
            $updDem->execute([$row['id_demande']]);
